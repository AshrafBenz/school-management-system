...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($errors)){
        try {
            $pdo->prepare("
                INSERT INTO eleve(matricule, nom, prenom, date_naissance, sexe, adresse, telephone, email, date_inscription, id_classe)
                VALUES(?, ?, ?, ?, ?, ?, ?, ?, CURDATE(), ?)
            ")->execute([$matricule, $nom, $prenom, $date_naissance, $sexe, $adresse, $telephone, $email, $id_classe]);

            $_SESSION['msg'] = "Élève $prenom $nom ajouté avec succès.";
...
<div class="page-header">
    <div>
        ...
        <p>Remplissez les informations du nouvel élève</p>
    </div>
                    ...
                    <select name="id_classe" class="form-control">
                        ...
                        <?php foreach($classes as $cl): ?>
                            <option value="<?php echo $cl['id_classe']; ?>"
                                <?php echo (isset($_POST['id_classe']) && $_POST['id_classe'] == $cl['id_classe']) ? 'selected' : ''; ?>>
